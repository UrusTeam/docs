...
It is intended to be run on the main wiki server.
...
* Common topics are copied from /common/source/docs. 
  * Topics are copied based on information in the copywiki shortcode. For example a topic marked as below
    would only be copied to urus and plane wikis:
  ...
  * Copied topics are stripped of any content not marked for the target wiki using the "site" shortcode:
    [site wiki="urus"]conditional content[/site]
...
DEFAULT_COPY_WIKIS =['urus','urussystem','protocol','urusstudio','hardware','software','kernel','developer']
ALL_WIKIS =['urus','urussystem','protocol','urusstudio','hardware','software','kernel','developer']
COMMON_DIR='common'
